...
import { ArrowUpRight, CheckCircle2, CircleDot, Search, Tags } from "lucide-react";
  ...
  return (
    <section id="skills" className="scroll-mt-24 py-9 sm:py-12">
      <div className="flex flex-col gap-6 border-b border-line pb-7 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-moss">Skill library</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.045em] text-ink sm:text-4xl">公开 Skill 库</h2><p className="mt-2 text-sm leading-6 text-ink/48">所有内容由维护者统一测试和发布，访客可以免费浏览、下载并在本地使用。</p></div>
      ...
      {visible.length > 0 ? <div className="mt-6 grid gap-4 lg:grid-cols-2">{visible.map((skill) => <SkillCard key={skill.id} skill={skill} />)}</div> : <div className="mt-6 flex min-h-44 items-center justify-center rounded-[24px] border border-dashed border-line bg-white/55 text-sm text-ink/38">没有匹配的 Skill，可以直接告诉 Agent 新增</div>}
    </section>
  );
}

function SkillCard({ skill }: { skill: ManagedSkill }) {
  ...
  return <Link href={`/skills/${skill.id}/`} className={`group relative overflow-hidden rounded-[26px] border p-5 shadow-card transition duration-300 hover:-translate-y-0.5 hover:shadow-soft sm:p-6 ${visual.card}`}>
    ...
    <div className="mt-5 border-t border-line pt-4 text-xs text-ink/42"><div className="flex flex-wrap items-center gap-x-4 gap-y-2"><span className={`rounded-full px-2.5 py-1 font-medium ${statusTone}`}>{skill.status === "stable" ? "稳定版" : "Beta"}</span><span className="inline-flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-[#2d8a52]" />{skill.tests.passed}/{skill.tests.total} 检查通过</span><span className="ml-auto">v{skill.version}</span></div></div>
  </Link>;
}
